...
const Dashboard = () => {
    ...
    useEffect(() => {
        ...
        if (user?.token) {
            fetchStats();
            const interval = setInterval(fetchStats, 15000); // Poll every 15s to keep "Live System" true
        ...
        <div className="space-y-12 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
                <div>
                    ...
                    <h1 className="text-3xl md:text-4xl font-black uppercase italic tracking-tighter">Business <span className="text-zinc-300">Performance</span></h1>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
                {/* RECENT ORDERS (2/3) */}
                <div className="lg:col-span-2 bg-white border border-zinc-100 p-6 md:p-8 rounded-3xl">
                    ...
                    <div className="space-y-4">
                        {stats.recentOrders?.map(o => (
                            <div key={o._id} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 bg-zinc-50 rounded-xl hover:bg-zinc-100 transition cursor-pointer gap-2" onClick={() => navigate(`/admin/orders`)}>
                                <div>
...
const StatSimple = ({ title, value, icon: Icon }) => (
    <div className="bg-white border border-zinc-100 p-5 md:p-6 rounded-2xl flex items-center gap-4 hover:shadow-lg transition-all">
        <div className="p-3 bg-zinc-50 rounded-xl text-black">
            <Icon size={20} />
        </div>
        <div>
            <h3 className="text-xl md:text-2xl font-black tracking-tighter text-zinc-900">{value}</h3>
            <p className="text-[9px] md:text-[10px] font-bold uppercase tracking-widest text-zinc-400">{title}</p>
        </div>
    </div>
);
