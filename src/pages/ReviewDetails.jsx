...
const ReviewDetails = () => {
    ...
    if (!reviewData) {
        return (
            <div className="flex flex-col items-center justify-center min-h-screen bg-black text-white p-6">
                ...
                <button onClick={() => navigate(-1)} className="bg-white text-black px-6 py-2 rounded-full text-xs font-black uppercase tracking-widest">Go Back</button>
            </div>
        );
    }
    ...
    return (
        <div className="fixed inset-0 z-[200] bg-black flex flex-col animate-in fade-in duration-300 overflow-y-auto no-scrollbar">
            {/* MOBILE HEADER */}
            <div className="fixed top-0 inset-x-0 z-[210] flex items-center justify-between p-4 bg-gradient-to-b from-black/80 to-transparent">
                <button 
                    onClick={() => navigate(-1)}
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <p className="text-[10px] font-black uppercase tracking-widest text-white leading-none mb-1">{reviewData.name}</p>
            ...
            {/* MEDIA SECTION */}
            <div className="w-full aspect-[3/4] md:aspect-auto md:h-screen bg-black relative flex items-center justify-center select-none overflow-hidden mt-16">
                {currentMedia ? (
                    ...
                    <>
                        ...
                        <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex gap-1.5 z-10 bg-black/40 backdrop-blur-md px-3 py-1.5 rounded-full border border-white/10">
                            {reviewData.media.map((_, idx) => (
                                <div
                                    key={idx}
                                    className={`w-1 h-1 rounded-full transition-all duration-300 ${idx === mediaIndex ? 'bg-white scale-125' : 'bg-white/20'}`}
                                />
                            ))}
                        </div>
                    </>
            ...
            {/* CONTENT SECTION */}
            <div className="w-full p-6 pb-24 bg-black flex flex-col flex-1">
                {/* PRODUCT REFERENCE */}
                <Link to={`/product/${reviewData.productSlug}`} className="flex items-center gap-3 p-3 mb-6 bg-white/5 rounded-2xl border border-white/10">
                    ...
                    <div className="min-w-0">
                        ...
                        <p className="text-[9px] font-black uppercase tracking-widest text-white truncate">{reviewData.productName}</p>
                    </div>
                    <ArrowUpRight size={14} className="ml-auto text-white/40" />
                </Link>
                ...
                <div className="space-y-4 mb-8">
                    ...
                    <p className="text-white leading-[1.8] text-[15px] font-medium whitespace-pre-line">
                        "{reviewData.comment}"
                    </p>
                </div>
                ...
                {reviewData.adminResponse && (
                    <div className="bg-white/5 p-6 rounded-[2rem] border border-white/10 relative overflow-hidden">
                        <p className="text-[9px] font-black uppercase tracking-widest text-white mb-4 flex items-center gap-2">
                            <span className="w-2 h-2 bg-white rounded-full animate-pulse" /> Official Response
                        </p>
                        <p className="text-[12px] text-zinc-400 leading-relaxed italic font-medium">{reviewData.adminResponse}</p>
                    </div>
                )}
            </div>
            
            {/* BOTTOM CTA */}
            <div className="fixed bottom-0 inset-x-0 p-4 bg-gradient-to-t from-black via-black/90 to-transparent">
                <Link 
                    to={`/product/${reviewData.productSlug}`}
                    className="w-full bg-white text-black py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2 shadow-2xl"
                >
