...
import { Plus, ChevronRight, MessageSquare, Clock, CheckCircle, AlertCircle, ArrowLeft } from 'lucide-react';
        ...
        <div className="min-h-screen bg-zinc-50 pt-40 pb-20 px-6">
            <div className="max-w-4xl mx-auto">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12">
                    <div>
                        <Link to="/account" className="flex items-center gap-2 text-zinc-400 text-[10px] font-black uppercase tracking-widest hover:text-black mb-2 transition-colors">
                            <ArrowLeft size={14} /> Back to Dashboard
                        </Link>
                        <h1 className="text-3xl md:text-4xl font-black uppercase italic tracking-tighter">My Tickets</h1>
                    </div>
                    <button onClick={() => setShowCreate(!showCreate)} className="bg-black text-white px-6 py-3 rounded-full flex items-center gap-2 text-xs font-bold uppercase tracking-widest hover:bg-zinc-800 transition-all">
                ...
                {showCreate && (
                    <div className="bg-white p-8 rounded-3xl border border-zinc-100 shadow-xl mb-12 animate-in slide-in-from-top-4">
                        <h3 className="font-black text-xl uppercase italic mb-6">Submit a Request</h3>
                        <form onSubmit={handleCreate} className="space-y-6">
                            <div>
                                <label className="text-[10px] font-black uppercase text-zinc-400 block mb-2">Subject / Issue</label>
                                <input required type="text" className="w-full bg-zinc-50 border border-zinc-200 p-4 rounded-xl font-bold outline-none focus:border-black"
                            ...
                            <div>
                                ...
                                <select className="w-full bg-zinc-50 border border-zinc-200 p-4 rounded-xl font-bold outline-none focus:border-black"
                            ...
                            <div>
                                ...
                                <textarea required className="w-full bg-zinc-50 border border-zinc-200 p-4 rounded-xl font-medium outline-none focus:border-black h-32 resize-none"
                                    placeholder="Describe your issue in detail..."
                                    value={newTicket.message} onChange={e => setNewTicket({ ...newTicket, message: e.target.value })} />
                            </div>
                            <button disabled={submitting} className="w-full bg-black text-white py-4 rounded-xl font-black uppercase tracking-widest text-xs">
                ...
                <div className="space-y-4">
                    {tickets.length === 0 ? (
                        <div className="text-center py-20 bg-white rounded-3xl border border-zinc-100">
                            ...
                            <h3 className="text-lg font-bold">No tickets yet</h3>
                            <p className="text-zinc-500 text-sm">Need help? Create a new support ticket.</p>
                        </div>
                    ) : (
                        tickets.map(ticket => (
                            <Link to={`/ticket/${ticket._id}`} key={ticket._id} className="block bg-white p-6 rounded-2xl border border-zinc-100 hover:shadow-lg transition-all group relative overflow-hidden">
