    ...
    <div className="fixed inset-0 z-[300] flex justify-end">
      ...
      <div className="relative w-full sm:max-w-[420px] bg-[#f8f8f8] h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-500">
        ...
        <div className="py-2.5 px-4 md:py-2 md:px-5 flex items-center justify-between bg-black text-white border-b border-white/10 shadow-lg shrink-0">
          <h2 className="font-black uppercase tracking-[0.2em]" style={{ fontSize: '7px' }}>Your Selection</h2>
          <button onClick={() => toggleCart(false)} className="p-1.5 hover:bg-white/10 rounded-full transition"><X size={16} /></button>
        </div>

        <div className="flex-1 overflow-y-auto no-scrollbar p-4 md:p-3 space-y-6 md:space-y-4 bg-zinc-50/50">
          {cartItems.length > 0 ? (
            <>
              {/* ITEM CARDS */}
              {/* SCARCITY ALERT */}
              ...
              <div className="space-y-3 md:space-y-2">
                {cartItems.map((item) => (
                  <div key={item._id || `${getItemId(item)}-${item.selectedVariant?.size}-${item.selectedVariant?.color}`} className="bg-white p-3 md:p-2.5 rounded-2xl md:rounded-xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-zinc-100/80 flex gap-3 md:gap-2.5 group hover:border-black transition-all duration-300">
                    ...
                    <div className="flex-1 flex flex-col justify-between py-0.5">
                      <div>
                        <div className="flex justify-between items-start gap-1">
                          <p className="font-black text-[11px] md:text-[9px] uppercase tracking-normal md:tracking-wider text-black leading-tight flex-1 line-clamp-2">{item.name || "Unknown Product"}</p>
                          <Price amount={item.price} className="font-black text-[10px] md:text-[9px] shrink-0 text-black px-1.5 py-0.5 bg-zinc-100 rounded" />
                        </div>
              ...
              <div className="bg-white p-4 md:p-3 rounded-2xl md:rounded-xl border border-zinc-100 shadow-sm space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  {appliedCoupon && (
                    <button onClick={handleRemoveCoupon} className="text-[9px] md:text-[8px] text-red-500 font-bold uppercase tracking-wider hover:underline">Remove</button>
                  )}
                </div>
                ...
                {appliedCoupon ? (
                  <div className="p-3 md:p-2 bg-green-50 border border-green-100 rounded-xl md:rounded-lg flex justify-between items-center">
                    ...
                    <Price amount={appliedCoupon?.discount || 0} className="text-[11px] md:text-[9px] font-bold text-green-600" />
                  </div>
                ) : (
                  <div className="flex gap-2">
                    <input
                      ...
                      className="flex-1 bg-[#f8f8f8] border border-transparent rounded-xl md:rounded-lg px-3 py-3 md:px-3 md:py-2 text-[10px] md:text-[8px] font-black uppercase tracking-normal md:tracking-widest outline-none focus:border-zinc-200 transition-all"
                    />
                    <button
                      onClick={handleApplyCoupon}
                      className="bg-black text-white px-4 md:px-4 rounded-xl md:rounded-lg text-[9px] md:text-[8px] font-black uppercase tracking-normal md:tracking-widest active:scale-95 transition-transform"
                    >
                      Apply
                    </button>
                  </div>
        ...
        {cartItems.length > 0 && (
          <div className="p-3 md:p-3 bg-[#fdfdfd] border-t border-zinc-200/60 space-y-3 md:space-y-2 shadow-[0_-10px_40px_rgba(0,0,0,0.03)] shrink-0">
              ...
              <div className="space-y-1.5">
                <div className="flex justify-between items-center text-[10px] md:text-[8px] font-black uppercase tracking-widest">
                  {subtotal >= siteSettings.freeShippingThreshold ? (
                    <span className="text-green-600 flex items-center gap-1"><CheckCircle2 size={9} /> You've unlocked FREE Shipping!</span>
                  ) : (
            ...
            {/* LOYALTY EARNINGS */}
            {user && (
              <div className="bg-amber-50/50 border border-amber-100/50 p-1.5 rounded-lg flex items-center justify-between">
                ...
                <span className="text-[7px] md:text-[8px] font-bold text-amber-900">
            ...
            <div className="space-y-2">
              ...
              <div className="flex justify-between items-end pt-1">
                ...
                <div className="text-right">
                  ...
                  <span className="text-[8px] text-zinc-400 font-medium uppercase tracking-widest">Incl. of all taxes</span>
                </div>
              </div>
            </div>
            <button
              ...
              className="w-full bg-black text-white py-2.5 md:py-2.5 rounded-xl md:rounded-lg font-black uppercase tracking-[0.2em] text-[10px] md:text-[8px] flex items-center justify-center gap-2 active:scale-[0.98] hover:bg-zinc-900 transition-all shadow-xl shadow-black/10 group px-4"
            >
              <span>Secure <span className="text-amber-400 group-hover:text-white transition-colors">Checkout</span></span> <ChevronRight className="w-3.5 h-3.5 md:w-4 md:h-4 group-hover:translate-x-1 transition-transform" />
            </button>
          </div>
        )}
      </div>
