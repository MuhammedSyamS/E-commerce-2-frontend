...
const PaymentMethods = ({ onSelect }) => {
  ...
  const methods = [
    ...
    { id: 'cod', name: 'Cash on Delivery', icon: <Wallet className="w-5 h-5" />, description: 'Pay when your order arrives' },
  ];
    ...
    <div className="space-y-4">
      ...
      {methods.map((method) => (
        <div
          ...
          className={`relative flex items-center p-5 border-2 cursor-pointer transition-all duration-300 rounded-xl ${selected === method.id
